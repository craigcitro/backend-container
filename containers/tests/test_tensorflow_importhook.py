...
from ipython_startup_scripts import _001tensorflowimporthook
...
class TensorFlowImportHookTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # The TensorFlow module interacts poorly with calls to reload(). Use a
    # placeholder module instead since we're mostly concerned with testing that
    # the custom import hook is registered and run.
    if 'placeholder' in sys.modules:
      del sys.modules['placeholder']

  @mock.patch.object(_001tensorflowimporthook._TensorFlowImportHook,
                     'load_module')
  @mock.patch.object(
      _001tensorflowimporthook._TensorFlowImportHook,
      '_has_gpu',
      return_value=True)
  def testDoesNothingWithEnvVariableSet(self, unused_mock_has_gpu,
                                        mock_load_module):
    os.environ['DISABLE_COLAB_TF_IMPORT_HOOK'] = '1'
    _001tensorflowimporthook.register_hook(module_name='placeholder')
    ...
    # skipped.
    self.assertIn('placeholder', sys.modules)
  ...
  def testRunsInitCodeOnImportWithFailure(self, unused_mock_has_gpu):
    ...
    # Relevant TensorFlow code is not present in the placeholder module. This
    ...
    reload_module(sys.modules['placeholder'])
    self.assertNotIn('COLAB_TF_IMPORT_HOOK_STARTUP_DURATION', os.environ)
